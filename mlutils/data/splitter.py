...
class Splitter:
    ...
    def _validate_splits(self, splits):
        assert all(k in splits for k in defaults.LEARNING_MODES), f'Split names must be equal to {defaults.LEARNING_MODES}.'
        assert all(isinstance(splits[k], list) for k in defaults.LEARNING_MODES), f'Splits must be lists.'
        assert all(len(splits[k]) > 0 for k in defaults.LEARNING_MODES), 'Splits length must be > 0.'
    ...
    def split(self, indices, stratification=None):
        ...
        splits = {defaults.TRAINING: [], defaults.VALIDATION: [], defaults.TEST: []}
        ...
        for outer_train_idx, outer_test_idx in outer_splitter:
            splits[defaults.TEST].append([indices[outer_test_idx].tolist()])
            ...
            splits[defaults.TRAINING].append(train_inner_folds)
            splits[defaults.VALIDATION].append(val_inner_folds)
    ...
    def get_split(self, partition, outer_fold=0, inner_fold=0):
        ...
        assert partition in defaults.LEARNING_MODES, f'Unknown partition {partition}.'
